	...
	export default {
		...
		methods:{
			save(){
				...
				if(this.item.add){
              		this.$set(this.$parent.addList, this.item.index, obj);
				}else{
					this.$set(this.$parent.editList, this.item.index, obj);
				}
				console.log(this.$parent.editList,'this.$parent.editList')
				this.show = false;
			},
			dragover(e){
