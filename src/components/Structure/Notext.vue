    ...
    <div class="main">
      ...
      <div class="main_bottom">
        <div class="tab_header">
          <ul>
            <li class="tab1 on" @click="toggleTab('tab1')"><i class="icon_tab1"></i>已完成知识链</li>
            <li class="tab2" @click="toggleTab('tab2')"><i class="icon_tab2"></i>查看知识链图谱</li>
          </ul>
          <span>操作</span>
        </div>
        <div class="tab_content">
          <div v-if="tabs.tab1" class="tab1" v-loading="chain_loading">
            <ul>
          ...
          <div v-if="tabs.tab2" class="tab2" v-loading="chain_loading">
              <create v-if="loadingGraph" :list="graphChainList" :visible="true" :editable='false'></create>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  mounted(){
    if(this.$store.state.transferList){
      this.List = this.$store.state.transferList.list;
    }
           ...
           .then((res) => {
            if(res.data.code == 0){
              ...
              res.data.data.map((chain,index) => {
                const data = [];
                chain.knowledge_linked.map((item, i) => {
                  data.push({
                    name : item.name,
                    type : item.type,
                ...
                this.chainList.push({
                  id : chain._id, //知识链id
                  owner : chain.user_zh_name,
                  time : chain.write_date,
                  data : data
                });

                var graphData = {};
                var len = chain.knowledge_linked.length;
                for (var i = 0; i < len; i++) {
                  var elem = chain.knowledge_linked[i];
                  graphData = {
                    name: elem.name,
                  ...
                  if (graphData.category == 'relation') {
                    graphListData.from = chain.knowledge_linked[i - 1]._id;
                    graphListData.to = chain.knowledge_linked[i + 1]._id;
                  }
